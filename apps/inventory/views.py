...
from .serializers import (
    ...
    ProductStockSerializer, ProductTransferLogSerializer, ProductTransferActionSerializer
)
...
class ProductViewSet(viewsets.ModelViewSet):
    queryset = Product.objects.select_related('supplier', 'created_by').all()
    serializer_class = ProductSerializer
    permission_classes = [IsAuthenticated, IsAdminUserRole | IsWarehouseManagerRole]


    @action(detail=False, methods=['post'], url_path='transfer-stock', serializer_class=ProductTransferActionSerializer)
    def transfer_stock(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(ProductTransferLogSerializer(log_entry).data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

class ProductStockViewSet(viewsets.ModelViewSet):
    queryset = ProductStock.objects.select_related('product', 'warehouse', 'product__supplier').all()
